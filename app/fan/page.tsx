      ...
      <div className={`mx-auto min-h-dvh max-w-6xl border-x ${isLight ? "border-slate-200 bg-[linear-gradient(180deg,#f8fbff_0%,#eef5ff_100%)]" : "border-white/5 bg-[radial-gradient(circle_at_70%_-20%,rgba(112,0,255,.19),transparent_33%),linear-gradient(180deg,#111318_0%,#0c0e12_45%,#111318_100%)]"}`}>
        <header className="flex items-center justify-between px-5 py-4 sm:px-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setLanguageOpen(!languageOpen)} aria-label="Change language" aria-expanded={languageOpen} className="relative flex h-9 items-center gap-1 rounded-full border border-white/10 bg-white/5 px-3 text-xs font-medium hover:bg-white/10"><Languages className="h-4 w-4" /> EN</button>
            {languageOpen && <div className="absolute right-20 top-14 z-10 rounded-xl border border-white/10 bg-[#16181d] p-2 text-xs shadow-xl"><button className="block rounded-lg px-3 py-2 hover:bg-white/10">English</button><button className="block rounded-lg px-3 py-2 hover:bg-white/10">Español</button><button className="block rounded-lg px-3 py-2 hover:bg-white/10">Français</button></div>}
            <button aria-label="Notifications" className="relative rounded-full p-2 text-slate-300 hover:bg-white/10"><Bell className="h-5 w-5" /><span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-primary" /></button>
            <UserProfileDropdown />
          </div>
        </header>
        ...
        <section className="px-5 pb-5 pt-4 sm:px-8">
          ...
          <div className="mt-5 flex items-center gap-3 overflow-hidden rounded-2xl border border-white/10 bg-white/[.06] p-3">
            ...
            <div className="min-w-0 flex-1"><p className="text-sm font-semibold">USA <span className="mx-2 text-slate-400">vs</span> Argentina</p><p className="mt-0.5 text-xs text-slate-400">MetLife Stadium · Group Stage · Today, 8:00 PM</p></div>
        ...
        <section className="px-5 pb-5 sm:px-8">
          <div className="relative overflow-hidden rounded-3xl border border-[#7000ff]/45 bg-[linear-gradient(125deg,rgba(45,20,74,.95),rgba(17,19,24,.98))] p-5 shadow-[0_12px_45px_rgba(112,0,255,.14)] sm:p-6">
            ...
            {showAnswer && <div className="relative mt-4 rounded-2xl bg-black/20 p-4 text-sm leading-6 text-slate-200"><button onClick={() => setShowAnswer(false)} aria-label="Dismiss answer" className="float-right text-slate-400"><X className="h-4 w-4" /></button><span className="font-semibold text-primary">Here’s your route:</span> From your current location, follow the teal signs to Concourse East. Take the stairs by Gate B, then turn left at Section 122. You’ll reach your seat in about 6 minutes.</div>}
            <div className="relative mt-5 flex gap-2"><input value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => e.key === "Enter" && ask()} placeholder="Try “How do I get to my seat?”" className="h-12 min-w-0 flex-1 rounded-xl border border-white/10 bg-black/30 px-4 text-sm outline-none placeholder:text-slate-400 focus:border-primary/60" /><button onClick={() => ask()} aria-label="Send question" className="grid h-12 w-12 place-items-center rounded-xl bg-primary text-[#071225] hover:bg-primary/90"><ChevronRight className="h-5 w-5" /></button><button aria-label="Voice input" className="grid h-12 w-12 place-items-center rounded-xl border border-white/10 bg-white/5 text-slate-200 hover:bg-white/10"><Mic className="h-5 w-5" /></button></div>
